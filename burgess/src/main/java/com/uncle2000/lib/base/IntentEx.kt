...
// Intent extensions
fun Intent.putSerializableList(key: String, list: List<Serializable>?) = list?.let { putExtra(key,
    SerializableListHolder(list)
) }
...
// Bundle extensions
fun Bundle.putSerializableList(key: String, list: List<Serializable>?) = list?.let { putSerializable(key,
    SerializableListHolder(list)
) }
